...
function init(e) {
    ...
    logoSize = [980, 404, 954, 294]; // logo w-h rel w-h logo-pre w-h
...
function prepareVideo()
{
    ...
    else
    {
        ...
        for (var i=0; i<bgImages.length; i++)
        {
            ...
            // change first autoAlpha to "0" to show transition
            bgImgTL.insert(TweenLite.to(img, .3, { autoAlpha: 1, ease: Linear.easeNone }), (i+1)*bgImgTime);
...
function enterSite() {
    ...
    var enterTL = new TimelineMax({ paused: true, immediateRender: true })
        ...
        .insert(TweenLite.to(releaseDate,.1, { display: 'block', marginTop: logoSize[1]*homeLogoScale * .18, ease: Strong.easeIn }), .6)
...
function handleResize(e) {
    ...
    releaseDate.css({ marginTop: logoSize[1]*logoScale * .18,  width: logoSize[2]*logoScale, height: logoSize[3]*logoScale, 'margin-left': -logoSize[2]*logoScale * .5 });
